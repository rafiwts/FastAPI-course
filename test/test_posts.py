...
def test_unautorized_user_get_one_posts(client, test_posts):
...
@pytest.mark.parametrize("title, content, published", [
    ("awseome new title", "awesome new content", True),
...
def test_create_post_default_published_true(authorized_client, test_user, test_posts):
    res = authorized_client.post(
        "/posts/", 
        json={"title": "an example of a title", "content": "an example of the content"}
    )

    created_post = schema.Post(**res.json())
    assert res.status_code == 201
    assert created_post.title == "an example of a title"
    assert created_post.content == "an example of the content"
    assert created_post.published == True
    assert created_post.user_id == test_user['id']


def test_create_post_default_published(client, test_user, test_posts):
    res = client.post(
        "/posts/", 
        json={"title": "an example of a title", "content": "an example of the content"}
    ) 
...
def test_update_post_non_exist(authorized_client, test_user, test_posts):
    ...
    res = authorized_client.put("/posts/8000", json=data)
    assert res.status_code == 404
